...
import tutorial.rs.buffer.Buffer;
import tutorial.rs.buffer.util.ByteBufUtils;

import java.util.*;

public class Model {

    public static javafx.scene.paint.Color rs2HSBToColor(short hsb, int alpha) {
    ...
    public void convertTexturesToOldFormat() {
        ...
        if (IntStreamEx.of(faceMaterial).anyMatch(x -> x > 117)) {
    ...
    public byte[] encodeOld() {
        ...
        /*boolean scale = !oldModel;

        if (scale) {
            for (int i = 0; i < vertices; i++) {
                verticesXCoordinate[i] /= 4;
                verticesYCoordinate[i] /= 4;
                verticesZCoordinate[i] /= 4;
            }
        }*/
        ByteBuf masterBuffer = Unpooled.buffer(10000);
        ByteBuf vertexFlagsBuffer = Unpooled.buffer();
        ByteBuf faceTypesBuffer = Unpooled.buffer();
        ByteBuf faceIndexTypesBuffer = Unpooled.buffer();
        ByteBuf trianglePrioritiesBuffer = Unpooled.buffer();
        ByteBuf faceSkinsBuffer = Unpooled.buffer();
        ByteBuf vertexSkinsBuffer = Unpooled.buffer();
        ByteBuf faceAlphasBuffer = Unpooled.buffer();
        ByteBuf triangleIndicesBuffer = Unpooled.buffer();
        ByteBuf faceColorsBuffer = Unpooled.buffer();
        ByteBuf verticesXBuffer = Unpooled.buffer();
        ByteBuf verticesYBuffer = Unpooled.buffer();
        ByteBuf verticesZBuffer = Unpooled.buffer();
        ByteBuf texturesBuffer = Unpooled.buffer();
        ByteBuf footerBuffer = Unpooled.buffer();

        ByteBuf[] buffers = {vertexFlagsBuffer,
                faceIndexTypesBuffer, trianglePrioritiesBuffer,
                faceSkinsBuffer, faceTypesBuffer, vertexSkinsBuffer,
                triangleIndicesBuffer, faceAlphasBuffer,
                faceColorsBuffer,
        ...
        boolean hasVertexLabels = vertexWeights != null;
        ...
        for (int vertex = 0; vertex < vertices; vertex++) {
            ...
            int flag = 0;
            if (xOffset != 0) {
                ByteBufUtils.writeUnsignedSmart(xOffset, verticesXBuffer);
                flag |= 0x1;
            }
            if (yOffset != 0) {
                ByteBufUtils.writeUnsignedSmart(yOffset, verticesYBuffer);
                flag |= 0x2;
            }
            if (zOffset != 0) {
                ByteBufUtils.writeUnsignedSmart(zOffset, verticesZBuffer);
                flag |= 0x4;
            }

            vertexFlagsBuffer.writeByte(flag);

            verticesXCoordinate[vertex] = baseX + xOffset;
            verticesYCoordinate[vertex] = baseY + yOffset;
            verticesZCoordinate[vertex] = baseZ + zOffset;
            baseX = verticesXCoordinate[vertex];
            baseY = verticesYCoordinate[vertex];
            baseZ = verticesZCoordinate[vertex];
            if (hasVertexLabels) {
                int weight = vertexWeights[vertex];
                vertexSkinsBuffer.writeByte(weight);
            }
        }


        boolean hasTriangleInfo = triangleInfo != null;
        boolean hasTrianglePriorities = trianglePriorities != null;
        boolean hasTriangleAlpha = faceAlpha != null;
        boolean hasTriangleSkins = triangleSkin != null;

        System.out.println("has t skins = " + hasTriangleSkins);


        for (int face = 0; face < triangleCount; face++) {
            faceColorsBuffer.writeShort(triangleColors[face]);

            if (hasTriangleInfo) {
                faceTypesBuffer.writeByte(triangleInfo[face]);
            }

            if (hasTrianglePriorities) {
                trianglePrioritiesBuffer.writeByte(trianglePriorities[face]);
            }
            if (hasTriangleAlpha) {
                faceAlphasBuffer.writeByte(faceAlpha[face]);
            }

            if (hasTriangleSkins) {
                int weight = triangleSkin[face];
        ...
        for (int face = 0; face < triangleCount; face++) {
            int currentA = faceIndicesA[face];
            int currentB = faceIndicesB[face];
            int currentC = faceIndicesC[face];
            if (currentA == lastB && currentB == lastA && currentC != lastC) {
                faceIndexTypesBuffer.writeByte(4);
                ByteBufUtils.writeUnsignedSmart(currentC - pAcc, triangleIndicesBuffer);
                int back = lastA;
                lastA = lastB;
                lastB = back;
                pAcc = lastC = currentC;
                ...
                ByteBufUtils.writeUnsignedSmart(currentA - pAcc, triangleIndicesBuffer);
                ByteBufUtils.writeUnsignedSmart(currentB - currentA, triangleIndicesBuffer);
                ByteBufUtils.writeUnsignedSmart(currentC - currentB, triangleIndicesBuffer);
                lastA = currentA;
                lastB = currentB;
                pAcc = lastC = currentC;
            }
        ...
        for (int face = 0; face < texturedFaces; face++) {
            texturesBuffer.writeShort(textureVertexA[face]);
            texturesBuffer.writeShort(textureVertexB[face]);
            texturesBuffer.writeShort(textureVertexC[face]);
        }

        footerBuffer.writeShort(vertices);
        footerBuffer.writeShort(triangleCount);
        footerBuffer.writeByte(texturedFaces);

        footerBuffer.writeByte(hasTriangleInfo ? 1 : 0);
        footerBuffer.writeByte(hasTrianglePriorities ? -1 : modelPriority);
        ...
        footerBuffer.writeShort(verticesXBuffer.writerIndex());
        footerBuffer.writeShort(verticesYBuffer.writerIndex());
        footerBuffer.writeShort(verticesZBuffer.writerIndex());
        footerBuffer.writeShort(triangleIndicesBuffer.writerIndex());

        for (ByteBuf buf : buffers) {
            masterBuffer.writeBytes(buf);
        }
        //  Arrays.stream(buffers).forEachOrdered(masterBuffer::writeBytes);
        byte[] data = new byte[masterBuffer.writerIndex()];
        masterBuffer.readBytes(data);
